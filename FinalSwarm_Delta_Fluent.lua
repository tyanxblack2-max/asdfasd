...
local Rayfield, Fluent, window, tab, raid, lobby, misc, credits
local function wrapFluentTab(ftab)
    local api = {}
    function api:CreateSection(a) pcall(function() ftab:AddSection(a.name or "") end) end
    function api:CreateToggle(a)
        local t = ftab:AddToggle(a.name, {Title=a.name, Default=a.value or false, Callback=function(v) pcall(a.callback, v) end})
        return {Set=function(_,v) pcall(function() t:SetValue(v) end) end}
    end
    function api:CreateSlider(a)
        local s = ftab:AddSlider(a.name, {Title=a.name, Default=a.value or a.range[1], Min=a.range[1], Max=a.range[2], Rounding=0, Callback=function(v) pcall(a.callback, v) end})
        return {Set=function(_,v) pcall(function() s:SetValue(v) end) end}
    end
    function api:CreateButton(a) pcall(function() ftab:AddButton({Title=a.name, Description="", Callback=function() pcall(a.callback) end}) end) end
    function api:CreateDropdown(a)
        local vals = a.options or {}
        local def = (a.value and a.value[1]) or vals[1]
        local d = ftab:AddDropdown(a.name, {Title=a.name, Values=vals, Multi=a.multiSelect or false, Default=def})
        if a.callback and d and d.OnChanged then pcall(function() d:OnChanged(function(v) pcall(a.callback, type(v)=="table" and v or {v}) end) end) end
        return {value=a.value or {}, Set=function()end}
    end
    function api:CreateStat(a)
        local p = ftab:AddParagraph({Title=a.name, Content=tostring(a.value)})
        return {Set=function(_,v) pcall(function() p:SetDesc(tostring(v)) end) end}
    end
    return api
end
if IS_DELTA then
    print("[Delta] try Fluent (beautiful mobile)")
    local okF, resF = pcall(function()
        if setthreadidentity then pcall(setthreadidentity, 8) end
        local src = safeHttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua")
        if not src or #src < 1000 then src = safeHttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/src/Fluent.lua") end
        if not src then error("no fluent") end
        return loadstring(src)()
    end)
    if okF and resF and type(resF.CreateWindow)=="function" then
        Fluent = resF
        local okW, win = pcall(Fluent.CreateWindow, Fluent, {Title="Liquid Hub", SubTitle="Auto Farm", TabWidth=160, Size=UDim2.fromOffset(580,460), Acrylic=false, Theme="Dark", MinimizeKey=Enum.KeyCode.LeftControl})
        if okW and win then
            window = {Notify=function(_,a) pcall(function() Fluent:Notify({Title=a.title or "Liquid", Content=a.content or "", Duration=3}) end) end}
            -- wrap fluent window tabs to Rayfield-like API
            local function safeFluentTab(name, icon)
                local okT, ftab = pcall(win.AddTab, win, {Title=name, Icon=icon or ""})
                if okT and ftab then return wrapFluentTab(ftab) end
                return {CreateToggle=function() return {Set=function()end} end, CreateSlider=function() return {Set=function()end} end, CreateSection=function()end, CreateButton=function()end, CreateDropdown=function() return {value={}} end, CreateStat=function() return {Set=function()end} end}
            end
            tab = safeFluentTab("Main","home")
            raid = safeFluentTab("Raid","swords")
            lobby = safeFluentTab("Lobby","package")
            misc = safeFluentTab("Misc","settings")
            credits = safeFluentTab("Credits","info")
            pcall(function() win:SelectTab(1) end)
            print("[Delta] Fluent loaded")
        else
            print("[Delta] Fluent window failed", okW)
        end
    else
        print("[Delta] Fluent load failed", okF, type(resF))
    end
else
    -- PC: try Rayfield
    do
...
-- Light fallback if both failed (staggered PlayerGui to avoid Rcoin)
if not window then
    print("[Delta] light fallback")
    local parent = game.Players.LocalPlayer:FindFirstChild("PlayerGui")
    local sg = Instance.new("ScreenGui")
    sg.Name = "LiquidHub_Light"
    ...
    task.wait(0.05)
    local function apiFor(prefix)
        local api = {}
        function api:CreateSection(a) local l=Instance.new("TextLabel") l.Text=prefix.." "..(a.name or "") l.Size=UDim2.new(1,0,0,16) l.BackgroundTransparency=1 l.TextColor3=Color3.fromRGB(180,180,180) l.Font=Enum.Font.Gotham l.TextSize=12 l.Parent=scroll task.wait(0.02) end
        function api:CreateToggle(a)
            local b=Instance.new("TextButton") b.Text=a.name..": "..(a.value and "ON" or "OFF") b.Size=UDim2.new(1,0,0,30) b.BackgroundColor3=a.value and Color3.fromRGB(0,170,90) or Color3.fromRGB(50,50,55) b.TextColor3=Color3.new(1,1,1) b.Font=Enum.Font.Gotham b.TextSize=13 b.Parent=scroll
            local state=a.value or false
            local function upd() b.Text=a.name..(state and ": ON" or ": OFF") b.BackgroundColor3=state and Color3.fromRGB(0,170,90) or Color3.fromRGB(50,50,55) end
        ...
        function api:CreateSlider(a)
            ...
            local tl=Instance.new("TextLabel") tl.Text=a.name..": "..cur tl.Size=UDim2.new(1,0,0,18) tl.BackgroundTransparency=1 tl.TextColor3=Color3.new(1,1,1) tl.Font=Enum.Font.Gotham tl.TextSize=12 tl.Parent=scroll
            local row=Instance.new("Frame") row.Size=UDim2.new(1,0,0,26) row.BackgroundTransparency=1 row.Parent=scroll
            local minus=Instance.new("TextButton",row) minus.Text="-" minus.Size=UDim2.new(0,40,1,0) minus.BackgroundColor3=Color3.fromRGB(70,70,75) minus.TextColor3=Color3.new(1,1,1)
            local plus=Instance.new("TextButton",row) plus.Text="+" plus.Size=UDim2.new(0,40,1,0) plus.Position=UDim2.new(1,-40,0,0) plus.BackgroundColor3=Color3.fromRGB(70,70,75) plus.TextColor3=Color3.new(1,1,1)
        ...
        function api:CreateButton(a) local b=Instance.new("TextButton") b.Text=a.name b.Size=UDim2.new(1,0,0,30) b.BackgroundColor3=Color3.fromRGB(60,60,180) b.TextColor3=Color3.new(1,1,1) b.Parent=scroll b.MouseButton1Click:Connect(function() pcall(a.callback) end) task.wait(0.02) end
        function api:CreateDropdown(a) return {value=a.value or {}} end
        function api:CreateStat(a) local l=Instance.new("TextLabel") l.Text=a.name..": "..tostring(a.value) l.Size=UDim2.new(1,0,0,18) l.BackgroundTransparency=1 l.TextColor3=Color3.fromRGB(200,200,200) l.Font=Enum.Font.Gotham l.TextSize=12 l.Parent=scroll task.wait(0.02) return {Set=function(_,v) l.Text=a.name..": "..tostring(v) end} end
        return api
    end
