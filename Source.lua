...
function Nebula:CreateWindow(config)
    ...
    local function ClampToScreen()
        ...
        -- clamp on the .Position OFFSET, not AbsolutePosition: on executors the
        -- gui inset makes AbsolutePosition differ from the stored offset (measured
        -- 188px apart), and reading absolute / writing offset shifted the window
        -- UP by the inset delta on every clamp — resize dragged it to the top of
        -- the screen and every minimize/restore teleported it higher
        local p = Main.Position
        local cx, cy = p.X.Offset, p.Y.Offset
        local x = math.clamp(cx, w / 2 + 2, math.max(w / 2 + 2, abs.X - w / 2 - 2))
        local y = math.clamp(cy, h / 2 + 2, math.max(h / 2 + 2, abs.Y - h / 2 - 2))
    ...
    -- StartMinimized: skip the pop-in entirely and sit as a title bar. The
    -- minimize button's own toggle path restores normally from this state.
    self.StartMinimized = config.StartMinimized == true
    ...
    -- (minimized may already be true here: CreateWindow honors StartMinimized
    -- by skipping the pop-in and sitting as a bare title bar)
        ...
        function tab:CreateSection(title, column)
            ...
            function section:CreateToggle(text, default, callback, flag)
                ...
                return {
                    ...
                    GetVisible = function() return frame.Visible end,
                }
            end
            ...
            function section:CreateDropdown(text, options, default, callback, flag)
                ...
                local function rebuild()
                    ...
                    for i, opt in ipairs(options) do
                        ...
                        ob:SetAttribute("OptName", tostring(opt))
                        ob.Parent = scroll
                        ...
                        ob.MouseButton1Click:Connect(function()
                            ...
                            -- refresh the bullet markers in place (rebuild() would
                            -- destroy the row whose handler is running right now)
                            for _, c in ipairs(scroll:GetChildren()) do
                                if c:IsA("TextButton") then
                                    local name = c:GetAttribute("OptName") or c.Text
                                    local nowSel = (name == tostring(opt))
                                    c.Text = nowSel and ("\u{2022}  " .. name) or name
                                    c.TextColor3 = nowSel and Theme("Accent") or Theme("Text")
                                end
                            end
