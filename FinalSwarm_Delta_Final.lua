...
-- UltraLight on-demand for Delta: start with 1 button, full UI only on click to avoid Rcoin (Test5 clean)
if not window then
    print("[Delta] ultra-light on-demand")
    ...
    -- deferred API: queue Create calls, build UI only on click with stagger
    local queues = {Main={}, Raid={}, Lobby={}, Misc={}, Credits={}}
    local built = false
    local function makeDeferred(name)
        local q = queues[name]
        local api = {}
        function api:CreateSection(a) table.insert(q, {"section", a}) end
        function api:CreateToggle(a) local o={}; table.insert(q, {"toggle", a, o}) return {Set=function(_,v) if o._upd then o._upd(v) end end} end
        function api:CreateSlider(a) local o={}; table.insert(q, {"slider", a, o}) return {Set=function(_,v) if o._upd then o._upd(v) end end} end
        function api:CreateButton(a) table.insert(q, {"button", a}) end
        function api:CreateDropdown(a) local o={value=a.value}; table.insert(q, {"dropdown", a, o}) return o end
        function api:CreateStat(a) local o={}; table.insert(q, {"stat", a, o}) return {Set=function(_,v) if o._upd then o._upd(v) end end} end
        return api
    end
    window = {Notify=function(_,a) pcall(function() game.StarterGui:SetCore("SendNotification",{Title=a.title or "Liquid", Text=a.content or ""}) end) end}
    tab = makeDeferred("Main")
    raid = makeDeferred("Raid")
    lobby = makeDeferred("Lobby")
    misc = makeDeferred("Misc")
    credits = makeDeferred("Credits")
    -- expose for later script to use (the script will call tab:CreateToggle etc. which will queue)
    -- on click, build the real UI
    openBtn.MouseButton1Click:Connect(function()
        if built then sg.Enabled = not sg.Enabled; return end
        built = true
        openBtn.Text = "Loading..."
        task.wait(0.2)
        -- build beautiful scroll (Rayfield-like, but staggered)
        ...
        title.Text = "  Liquid Hub  •  Final Swarm"
        ...
        scroll.Position = UDim2.new(0,5,0,47)
        ...
        local function buildQueue(list)
            for _,e in ipairs(list) do
                local typ, a, o = e[1], e[2], e[3]
                if typ=="section" then
                    local l=Instance.new("TextLabel") l.Text=a.name or "" l.Size=UDim2.new(1,0,0,20) l.BackgroundTransparency=1 l.TextColor3=Color3.fromRGB(160,160,170) l.Font=Enum.Font.GothamBold l.TextSize=11 l.TextXAlignment=Enum.TextXAlignment.Left l.Parent=scroll
                    l.Text = "  "..string.upper(a.name)
                elseif typ=="toggle" then
                    local row=Instance.new("Frame") row.Size=UDim2.new(1,0,0,38) row.BackgroundColor3=Color3.fromRGB(36,36,42) row.Parent=scroll Instance.new("UICorner", row).CornerRadius=UDim.new(0,10)
                    local tl=Instance.new("TextLabel", row) tl.Text=a.name tl.Size=UDim2.new(1,-60,1,0) tl.Position=UDim2.new(0,12,0,0) tl.BackgroundTransparency=1 tl.TextColor3=Color3.new(1,1,1) tl.Font=Enum.Font.Gotham tl.TextSize=13 tl.TextXAlignment=Enum.TextXAlignment.Left
                    local track=Instance.new("Frame", row) track.Size=UDim2.new(0,44,0,22) track.Position=UDim2.new(1,-52,0.5,-11) track.BackgroundColor3=Color3.fromRGB(55,55,60) track.Parent=row Instance.new("UICorner", track).CornerRadius=UDim.new(1,0)
                    local thumb=Instance.new("Frame", track) thumb.Size=UDim2.new(0,18,0,18) thumb.Position=UDim2.new(0,2,0.5,-9) thumb.BackgroundColor3=Color3.new(1,1,1) Instance.new("UICorner", thumb).CornerRadius=UDim.new(1,0)
                    local btn=Instance.new("TextButton", row) btn.Size=UDim2.new(1,0,1,0) btn.BackgroundTransparency=1 btn.Text="" btn.Parent=row
                    local state=a.value or false
                    local function upd(v) if v~=nil then state=v end track.BackgroundColor3=state and Color3.fromRGB(0,170,90) or Color3.fromRGB(55,55,60) thumb.Position=state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9) end
                    o._upd = upd; upd()
                    btn.MouseButton1Click:Connect(function() state=not state upd() pcall(a.callback,state) end)
                    pcall(a.callback, state)
                elseif typ=="slider" then
                    local cur=a.value or 0
                    local card=Instance.new("Frame") card.Size=UDim2.new(1,0,0,56) card.BackgroundColor3=Color3.fromRGB(36,36,42) card.Parent=scroll Instance.new("UICorner", card).CornerRadius=UDim.new(0,10)
                    local tl=Instance.new("TextLabel", card) tl.Text=a.name tl.Size=UDim2.new(1,-70,0,18) tl.Position=UDim2.new(0,12,0,6) tl.BackgroundTransparency=1 tl.TextColor3=Color3.new(1,1,1) tl.Font=Enum.Font.Gotham tl.TextSize=13 tl.TextXAlignment=Enum.TextXAlignment.Left
                    local valLbl=Instance.new("TextLabel", card) valLbl.Text=tostring(cur) valLbl.Size=UDim2.new(0,50,0,18) valLbl.Position=UDim2.new(1,-60,0,6) valLbl.BackgroundTransparency=1 valLbl.TextColor3=Color3.fromRGB(180,180,180) valLbl.Font=Enum.Font.GothamBold valLbl.TextSize=13
                    local track=Instance.new("Frame", card) track.Size=UDim2.new(1,-24,0,6) track.Position=UDim2.new(0,12,0,32) track.BackgroundColor3=Color3.fromRGB(55,55,60) track.Parent=card Instance.new("UICorner", track).CornerRadius=UDim.new(1,0)
                    local fill=Instance.new("Frame", track) fill.Size=UDim2.new((cur-a.range[1])/math.max(1,a.range[2]-a.range[1]),0,1,0) fill.BackgroundColor3=Color3.fromRGB(0,170,90) Instance.new("UICorner", fill).CornerRadius=UDim.new(1,0)
                    local row=Instance.new("Frame", card) row.Size=UDim2.new(1,0,0,0) row.BackgroundTransparency=1 row.Parent=card
                    local minus=Instance.new("TextButton",card) minus.Text="−" minus.Size=UDim2.new(0,28,0,22) minus.Position=UDim2.new(0,12,0,32) minus.BackgroundTransparency=1 minus.TextColor3=Color3.fromRGB(120,120,130) minus.Font=Enum.Font.GothamBold minus.TextSize=16
                    local plus=Instance.new("TextButton",card) plus.Text="+" plus.Size=UDim2.new(0,28,0,22) plus.Position=UDim2.new(1,-40,0,32) plus.BackgroundTransparency=1 plus.TextColor3=Color3.fromRGB(120,120,130) plus.Font=Enum.Font.GothamBold plus.TextSize=16
                    -- overlay buttons for track area
                    minus.ZIndex=2; plus.ZIndex=2
                    local function set(v) cur=math.clamp(v,a.range[1],a.range[2]) valLbl.Text=tostring(cur) fill.Size=UDim2.new((cur-a.range[1])/math.max(1,a.range[2]-a.range[1]),0,1,0) pcall(a.callback,cur) end
                    o._upd = set
                    ...
                    pcall(a.callback, cur)
                elseif typ=="button" then
                    local b=Instance.new("TextButton") b.Text=a.name b.Size=UDim2.new(1,0,0,30) b.BackgroundColor3=Color3.fromRGB(60,60,180) b.TextColor3=Color3.new(1,1,1) b.Parent=scroll b.MouseButton1Click:Connect(function() pcall(a.callback) end)
                elseif typ=="dropdown" then
                    -- dropdown stub, not needed for Delta core
                elseif typ=="stat" then
                    local l=Instance.new("TextLabel") l.Text=a.name..": "..tostring(a.value) l.Size=UDim2.new(1,0,0,18) l.BackgroundTransparency=1 l.TextColor3=Color3.fromRGB(200,200,200) l.Font=Enum.Font.Gotham l.TextSize=12 l.Parent=scroll
                    o._upd = function(v) l.Text=a.name..": "..tostring(v) end
                end
                task.wait(0.04)
            end
        end
        -- build each tab's queue in order
        buildQueue(queues.Main)
        buildQueue(queues.Raid)
        buildQueue(queues.Lobby)
        buildQueue(queues.Misc)
        buildQueue(queues.Credits)
        openBtn.Text = "Close Hub"
        print("[Delta] hub built on-demand")
    end)
    -- also allow double-click to toggle visibility
end
